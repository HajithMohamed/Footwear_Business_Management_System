  ...
  <section class="mb-4">
    ...
    <?php if ($isIncremental): ?>
      <form method="post" action="<?= e(url('purchases/' . $purchase['id'] . '/arrival/count')) ?>" class="mt-3 space-y-2 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100">
        ...
        <select name="arrival_item_id" required class="w-full rounded-lg px-2.5 py-2 text-sm ring-1 ring-slate-200"><option value="">Choose product</option><?php foreach ($items as $item): ?><option value="<?= (int) $item['id'] ?>"><?= e(trim(($item['mapped_brand_name'] ?? $item['brand_name'] ?? '') . ' ' . ($item['art_no'] ?? '')) ?: 'Unnamed') ?> (<?= (int) $item['received_pairs'] ?>/<?= (int) $item['expected_pairs'] ?>)</option><?php endforeach; ?></select>

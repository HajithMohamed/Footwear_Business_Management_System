...
<form method="post" action="<?= e(url("customers/{$customer['id']}/payment")) ?>" enctype="multipart/form-data" class="pb-40 md:pb-24" x-data='paymentForm(<?= $paymentInit ?>)'>
  ...
  <div class="mb-4 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
    <div class="mb-5 flex items-center justify-between border-b border-slate-100 pb-4">
      ...
      <div class="text-right"><p class="text-[10px] font-bold uppercase tracking-wide text-slate-400">Outstanding</p><p class="text-lg font-bold text-red-600"><?= money($customer['outstanding_due']) ?></p></div>
    </div>
    ...
    <div x-show="method === 'cash'" class="mt-4">
      ...
      <input type="number" name="amount" x-model.number="cashAmount" step="0.01" min="0.01" :max="outstanding" :required="method === 'cash'" :disabled="method !== 'cash'" class="w-full rounded-xl border-0 bg-green-50 px-4 py-4 text-right text-2xl font-bold text-green-700 ring-1 ring-green-200">
    </div>
  ...
  <div class="mb-4 grid grid-cols-3 gap-2 rounded-2xl bg-slate-900 p-4 text-white shadow-sm">
    <div><p class="text-[9px] font-bold uppercase tracking-wide text-slate-400">Outstanding</p><p class="mt-1 text-xs font-bold" x-text="money(outstanding)"></p></div>
    ...
    <div class="text-right"><p class="text-[9px] font-bold uppercase tracking-wide text-slate-400">Remaining</p><p class="mt-1 text-sm font-bold" :class="total > outstanding ? 'text-red-300' : ''" x-text="money(outstanding - total)"></p></div>
  </div>
  ...
  <div class="fixed bottom-[69px] left-0 right-0 z-40 border-t border-slate-200 bg-white p-4 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] md:bottom-0 md:left-64">
    <div class="mx-auto flex max-w-3xl gap-3">
      ...
      <button type="submit" :disabled="total <= 0 || total > outstanding" class="flex h-12 flex-[2] items-center justify-center rounded-xl bg-green-600 text-lg font-bold text-white shadow-sm disabled:cursor-not-allowed disabled:opacity-50" x-text="method === 'cheque' && cheques.length > 1 ? 'Save ' + cheques.length + ' Cheques' : 'Save Payment'"></button>
    </div>
  </div>
</form>
...
function paymentForm(init) {
  return {
    ...
    money(value) { return 'Rs. ' + Math.max(0, Number(value || 0)).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}); },

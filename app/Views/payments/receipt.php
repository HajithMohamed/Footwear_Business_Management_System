...
<div class="mx-auto max-w-md">
  <div id="payment-receipt" class="rounded-3xl bg-white p-6 text-center shadow-sm ring-1 ring-slate-100">
    ...
    <dl class="mt-5 space-y-3 border-y border-slate-100 py-4 text-sm"><div class="flex justify-between"><dt class="text-slate-500">Payment date</dt><dd class="font-bold"><?= e($paymentDate) ?></dd></div><?php if ($payment['payment_method'] === 'cheque'): ?><div class="flex justify-between"><dt class="text-slate-500">Cheque number</dt><dd class="font-bold"><?= e($payment['cheque_number']) ?></dd></div><div class="flex justify-between"><dt class="text-slate-500">Cheque date</dt><dd class="font-bold"><?= e($formatDate($payment['cheque_date'] ?? null)) ?></dd></div><?php endif; ?><div class="flex justify-between"><dt class="text-slate-500"><?= $previous < 0 ? 'Previous credit' : 'Previous balance' ?></dt><dd><?= money(abs($previous)) ?></dd></div><div class="flex justify-between text-base"><dt class="font-bold"><?= $remaining < 0 ? 'Customer credit' : 'Remaining balance' ?></dt><dd class="font-bold text-brand-700"><?= money(abs($remaining)) ?></dd></div></dl>
...
<script>function sharePayment(){const text=<?= json_encode('Payment received from ' . $payment['customer_name'] . ': ' . money($payment['amount']) . ' on ' . $paymentDate . '. ' . ($remaining < 0 ? 'Customer credit: ' : 'Remaining balance: ') . money(abs($remaining))) ?>;if(navigator.share){navigator.share({title:'Payment Received',text});}else{navigator.clipboard.writeText(text).then(()=>alert('Payment summary copied.'));}}</script>

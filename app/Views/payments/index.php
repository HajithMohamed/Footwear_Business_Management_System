...
<div class="mt-3 flex gap-2">
  <a href="<?= e(url("customers/{$customer['id']}/payment")) ?>"
     class="flex flex-1 items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 py-2.5 text-center text-sm font-semibold text-white"><?= ui_icon('wallet', 'h-4 w-4') ?> Record payment</a>
  <a href="<?= e(url("customers/{$customer['id']}")) ?>"
     class="flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-slate-600"><?= ui_icon('note', 'h-4 w-4') ?> Ledger</a>
</div>

<?php if ($payments): ?>
  <div class="mt-4 space-y-2 pb-4">
    <?php foreach ($payments as $p): ?>
      <?php [$icon, $label] = $methodLabel[$p['payment_method']] ?? ['wallet', ucfirst($p['payment_method'])]; ?>
      <div class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100">
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <p class="flex items-center gap-2 text-sm font-semibold text-slate-800"><?= ui_icon($icon, 'h-4 w-4') ?> <?= e($label) ?></p>
...
<?php else: ?>
  <div class="mt-4 rounded-2xl bg-white p-8 text-center shadow-sm ring-1 ring-slate-100">
    ...
    <a href="<?= e(url("customers/{$customer['id']}/payment")) ?>"
       class="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-4 py-2 text-sm font-medium text-white"><?= ui_icon('wallet', 'h-4 w-4') ?> Record the first one</a>
  </div>
<?php endif; ?>
